# Debian 13 Template Bootstrap Course Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Deliver `proxmox/debian13-template`, a guided ISO-to-template bootstrap course with the standalone guide and honest two-clone acceptance protocol.
...
**Tech Stack:** Python 3.13, YAML, pytest, Typer, existing curriculum/session/validation services, Proxmox UI/node shell, Debian 13 (trixie) installer.
...
- `src/learnlab/collections/proxmox/courses/debian13-template/course.yaml`: ordered manifest.
- `src/learnlab/collections/proxmox/courses/debian13-template/lessons/00-prerequisites-and-safety/lesson.yaml`: prerequisites-and-safety lesson.
- `src/learnlab/collections/proxmox/courses/debian13-template/lessons/01-create-installer-vm/lesson.yaml`: create-installer-vm lesson.
- `src/learnlab/collections/proxmox/courses/debian13-template/lessons/02-install-debian13/lesson.yaml`: install-debian13 lesson.
- `src/learnlab/collections/proxmox/courses/debian13-template/lessons/03-configure-lab-access/lesson.yaml`: configure-lab-access lesson.
- `src/learnlab/collections/proxmox/courses/debian13-template/lessons/04-seal-and-convert/lesson.yaml`: seal-and-convert lesson.
- `src/learnlab/collections/proxmox/courses/debian13-template/lessons/05-test-two-clones/lesson.yaml`: test-two-clones lesson.
- `src/learnlab/collections/proxmox/courses/debian13-template/lessons/06-configure-provider/lesson.yaml`: configure-provider lesson.
- `tests/test_debian13_template_course.py`: course-specific contracts and negative knowledge-answer coverage.
...
- `docs/Debian13-Template-Guide.md`: standalone detailed guide, including all seven phases.
- `docs/course-validation/2026-09-10-debian13-template.md`: non-secret offline/live acceptance report.
...
**Files:** Create `src/learnlab/collections/proxmox/courses/debian13-template/course.yaml`, `src/learnlab/collections/proxmox/courses/debian13-template/lessons/00-prerequisites-and-safety/lesson.yaml`, `tests/test_debian13_template_course.py`; modify `tests/test_shipped_course_metadata.py` and `tests/test_cli.py`.
...
- [ ] **Step 1: Add course loading/boundary regression tests.** Use these imports/helpers and tests in `tests/test_debian13_template_course.py`:
...
- [ ] **Step 2: Run RED.** `.venv/bin/python -m pytest tests/test_debian13_template_course.py -q`; expected missing-course failure, not a collection/import error.

- [ ] **Step 3: Create a working one-lesson course, not empty future lessons.** Initial manifest:

```yaml
id: debian13-template
title: Build a Debian 13 (trixie) Template for LearnLab
environment:
  scope: none
lessons:
...
- [ ] **Step 1: Add RED tests for the new instructional contract.** Add to `tests/test_debian13_template_course.py`:

```python
def test_installer_teaches_target_specific_steps():
    text = lesson_text("install-debian13")
    for concept in ['netinst', 'manual partition', 'VERSION_ID', 'GRUB']:
...
- [ ] **Step 4: Add knowledge-answer cases and verify.** For each added text check, table a known accepted concept and at least one negated/unrelated answer. Execute the actual `equals` or full regex against those examples; never just assert that a regex string contains anchors. Test YAML with `validate_catalog`; run `.venv/bin/learnlab validate proxmox/debian13-template` and focused pytest. Review instructions and shell snippets without running guest commands.

- [ ] **Step 5: Commit the installer/access lessons, guide sections and tests.** Subject: `feat: teach debian13-template installation and lab access`.

### Task 3: Safe sealing and two-clone identity acceptance

**Files:** Add `src/learnlab/collections/proxmox/courses/debian13-template/lessons/04-seal-and-convert/lesson.yaml`, `src/learnlab/collections/proxmox/courses/debian13-template/lessons/05-test-two-clones/lesson.yaml`; extend manifest, guide and `tests/test_debian13_template_course.py`.
...
- [ ] **Step 1: Add RED safety regressions.** In `tests/test_debian13_template_course.py`:
...
Run focused tests; expect missing lessons. Add OS-specific regression cases for the sealing design, including the Debian missing-key unit dependency and prohibition on ConditionFirstBoot=yes.
...
- [ ] **Step 5: GREEN, review and commit.** Run focused tests and offline validation. Subject: `feat: teach safe debian13-template sealing and clone checks`.

### Task 4: Profile handoff, complete guide and installed-wheel coverage

**Files:** Add `src/learnlab/collections/proxmox/courses/debian13-template/lessons/06-configure-provider/lesson.yaml`; finish manifest, guide and `tests/test_debian13_template_course.py`; modify `README.md`, `tests/test_packaging.py`, and extend no-profile CLI coverage from Task 1 as needed.
...
def test_complete_lesson_order_and_handoff():
    assert [lesson.id for lesson in load_course().lessons] == ['prerequisites-and-safety', 'create-installer-vm', 'install-debian13', 'configure-lab-access', 'seal-and-convert', 'test-two-clones', 'configure-provider']
...
- [ ] **Step 3: Finish standalone guide and README entry.** The guide contains all course steps, terminal labels, actual commands, expected output and concrete troubleshooting. Cite the primary references listed in the spec, record version assumptions, and explicitly label untested procedures. README links it and shows `learnlab start proxmox/debian13-template --include-drafts` without a provider. Distinguish template creation from downstream live course certification.
...
- [ ] **Step 5: GREEN and commit.** Run focused course tests, relevant CLI tests, `tests/test_packaging.py`, and offline validation. Subject: `docs: finish debian13-template provider handoff and guide`.

### Task 5: Full offline gate, review and optional live acceptance

**Files:** Create `docs/course-validation/2026-09-10-debian13-template.md`; update this plan's checked steps only when completed. Modify `src/learnlab/collections/certifications.yaml` only after an approved successful live run.
...
.venv/bin/ruff check .
.venv/bin/mypy src
.venv/bin/learnlab validate proxmox/debian13-template
git diff --check
```
...
print(course_digest(Path("src/learnlab/collections/proxmox/courses/debian13-template")))
DIGEST
```
